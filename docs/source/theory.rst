...
Let's choose some lengthscale $a$ as the unit of distance for a system. Since Meep defines :math:`$c=1$`, :math:`$a$` (or :math:`$a/c$`)
is also the unit of time for the system. The frequency :math:`$f$` (which corresponds to a time dependence :math:`$e^{-i 2\pi f t}$`)
is defined in units of :math:`$c/a$` (and similarly :math:`$\omega$` is defined in units of :math:`2π$c/a$`). This means that :math:`$f = 1/T$`, meaning that
